...
using System.Linq; // NECESSÁRIO para a ordenação (OrderBy)
...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Time.timeScale != 0)
        {
            ...
            // --- Lógica de Interpolação de Cor do Timer (EXISTENTE) ---
            ...
            // --- Lógica de Formatação do Tempo (EXISTENTE) ---
        ...
        // --- Condição de Fim de Jogo (EXISTENTE) ---
    ...
    // --- Métodos de Início de Jogo (EXISTENTE) ---
    ...
    // --- Métodos de Vagas (EXISTENTE) ---
    public void EscolherProximaVaga()
    {
        // Lógica para redefinir tags (se a vaga anterior foi desabilitada e não destruída)
    ...
    private void UpdateRankingDisplay(RankingData ranking)
    {
        ...
        for (int i = 0; i < MaxRankingEntries; i++)
        {
            if (i < ranking.entries.Count)
            {
                ...
                // Saída final: Posição: Nome - Tempo (ex: 1º: Matheus - 1:03.44s)
    ...
    // Método para limpar o ranking (Útil para testes, pode ser removido depois)
